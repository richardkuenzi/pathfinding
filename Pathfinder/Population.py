...
class Population:
    def __init__ (self, win, maze, startX, startY, direction, endX, endY, mazeSize):
    ...
    def runMazeSimulation (self):
        for i in xrange(100):
            count = 0
            while (not self.population[i].atEnd()) and (count < self.mazeSize):
    ...
    def stats (self):
        ...
        for x in self.population:
            # if the goal was found write into garesul.txt
            if(x.fitness == 1):
                gaoutput = open("garesult.txt","a")
                gaoutput.write(str(runtime) + ",\n")
                gaoutput.close
                print("Goal found!")
                time.sleep(60)
            elif(x.fitness == 0):
                ...
                print("Goal found!")
                time.sleep(60)
        #disable for runtime tests
        # first test is done with view to find out at which fitness value the goal is found
        # second run without gui
        self.drawAll()
        time.sleep(1)
        self.undrawAll()
